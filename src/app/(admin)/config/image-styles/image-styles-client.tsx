...
interface ImageStylesClientProps {
  initialStyles: ImageStyle[];
}

export default function ImageStylesClient({ initialStyles }: ImageStylesClientProps) {
  ...
  const handleSave = async (id: string) => {
    ...
    setSuccess('');
    try {
      const result = await createImageStyle({
        id,
        label: editLabel,
        effects: editEffects,
      });
      if (!result.success) {
        setError(result.error || 'Failed to update image style');
        return;
      }
      setEditing(null);
  ...
  const handleDelete = async (id: string) => {
    ...
    setSuccess('');
    try {
      const result = await deleteImageStyle(id);
      if (!result.success) {
        setError(result.error || 'Failed to delete image style');
        return;
      }
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!result.success) {
        setError(result.error || 'Failed to create image style');
        return;
      }
      setShowAdd(false);
      setNewId('');
