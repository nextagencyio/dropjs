...
export default function TextFormatsPage() {
  ...
  useEffect(() => {
    fetchTextFormats()
      ...
      .catch((err) => {
        setError(err instanceof Error ? err.message : 'Failed to load text formats');
        setLoading(false);
      });
  }, []);
  ...
  const handleSave = async (id: string) => {
    ...
    setSuccess('');
    try {
      const updated = await updateTextFormat(id, {
        label: editLabel,
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    setSuccess('');
    try {
      const created = await createTextFormat({
        id: newId,
        label: newLabel,
