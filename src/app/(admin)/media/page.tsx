...
import type { FileData, MediaListMeta } from '@/lib/api-media';
import { fetchMediaList, uploadFile, deleteFile } from '@/lib/api-media';
...
export default function MediaLibraryPage() {
  ...
  const [meta, setMeta] = useState<MediaListMeta>({ total: 0, page: 1, limit: 24, pages: 0 });
  ...
  const loadFiles = useCallback(async () => {
    try {
      setLoading(true);
      const params: { mimetype?: string; search?: string; page?: number; limit?: number } = {
        page,
        limit: 24,
      };
      if (filter) params.mimetype = filter;
      if (search) params.search = search;
      const result = await fetchMediaList(params);
