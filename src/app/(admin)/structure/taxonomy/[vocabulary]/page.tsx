...
export default function TaxonomyTermList() {
  ...
  const [tree, setTree] = useState<TermTreeNode[]>([]);
  ...
  const loadTree = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [treeData, counts] = await Promise.all([
        fetchTermTree(vocabulary),
        fetchTermContentCounts(vocabulary).catch(() => ({})),
      ]);
      setTree(treeData);
  ...
  const handleDelete = async (tid: number, title: string) => {
    ...
    try {
      await deleteEntity('taxonomy_term', vocabulary!, tid);
      loadTree();
    } catch {
  ...
  async function saveOrder(newTree: TermTreeNode[]) {
    ...
    try {
      ...
      await reorderTerms(vocabulary, updates);
      setTree(newTree);
    } catch {
